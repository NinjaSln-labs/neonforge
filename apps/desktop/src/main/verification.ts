 ...
 * detached 进程组：清理走 kill(-pid) 杀整组（与 tools.ts bash 执行同模式——坑 54：&/nohup 后代不留孤儿） */
function runOne(command: string, cwd?: string): Promise<VerificationResult> {
  return new Promise((resolve) => {
    const child = spawn(command, {
      cwd,
      shell: true,
      detached: true, // 新进程组——超时/结束时 kill 进程组（含后台后代）
    ...
    const killGroup = () => {
      try {
        process.kill(-child.pid!, 'SIGKILL') // 杀整个进程组（负 PID）
      } catch {
        /* 已退出——无残留 */
      }
    }
