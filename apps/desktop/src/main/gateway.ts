...
export const TOOL_DEFS = [
  ...
  // 2026-08-07 无阶段重构 S5：语义更新——批量批准入口统一（执行方案/动手前清单批准，不绑阶段）
  { type: 'function', function: { name: 'plan_approval', description: '规划级授权（批量批准入口）：动手前，把本次要新增/修改的文件清单一次性请求用户批准——用户批准后这些文件自动放行（write/edit 不再逐个问）。在给出执行方案、准备动手写文件之前调用一次（列出全部计划文件 + 各自原因）；中途有新文件要改，再调一次补充。', parameters: { type: 'object', properties: { summary: { type: 'string', description: '一句话说明本次要做什么（如「黑屏修复：补 DOM 元素」）' }, files: { type: 'array', items: { type: 'object', properties: { path: { type: 'string', description: '文件路径（绝对路径或项目内相对路径）' }, reason: { type: 'string', description: '为什么新增/修改这个文件（一句话）' } }, required: ['path', 'reason'] } } }, required: ['summary', 'files'] } } },
