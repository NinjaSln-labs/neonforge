...
export const TOOL_DEFS = [
  ...
  { type: 'function', function: { name: 'bash', description: '执行命令（需 L3 授权）——启动 dev server（vite / npm run dev / next dev 等）时**不要绑定固定端口**：不指定 --port（vite 端口被占自动递增）或用 --port 0（系统分配）；5173/5175 是 NeonForge（本应用）自己的保留端口（宿主 dev server / 测试 server）——看到它们有服务是宿主本身，不是你的项目服务：**不要 kill、不要占用、不要把它当你的服务地址告诉用户**。起服务后读实际输出里的地址（如 Local: http://localhost:5174/）或 lsof/curl 确认实际端口，把真实地址告诉用户，不要猜端口。', parameters: { type: 'object', properties: { command: { type: 'string' } }, required: ['command'] } } },
  ...
  // 2026-08-04 规划级授权（用户「设计/开发规划好文件后一次性要授权，减少逐个授权打断」）：模型动手前把本次任务要新增/修改的文件清单一次性请求批准
  { type: 'function', function: { name: 'plan_approval', description: '规划级授权：开始动手前，把本次任务要新增/修改的文件清单一次性请求用户批准——用户批准后这些文件自动放行（write/edit 不再逐个问）。在设计/开发阶段动手写文件之前调用一次（列出全部计划文件 + 各自修改原因）；如果中途有新文件要改，再调一次补充。', parameters: { type: 'object', properties: { summary: { type: 'string', description: '一句话说明本次要做什么（如「黑屏修复：补 DOM 元素」）' }, files: { type: 'array', items: { type: 'object', properties: { path: { type: 'string', description: '文件路径（绝对路径或项目内相对路径）' }, reason: { type: 'string', description: '为什么新增/修改这个文件（一句话）' } }, required: ['path', 'reason'] } } }, required: ['summary', 'files'] } } },
  ...
  // 2026-08-06 环境单源（尽调调研 5 源驱动）：开发阶段第 0 步 check-env 确认环境就绪（runtime/依赖/工具链）——写代码/起服务前先调
  { type: 'function', function: { name: 'check-env', description: '检测项目环境（开发阶段第 0 步：确认 runtime/依赖/工具链就绪）。返回 node 版本、node_modules 是否安装、可用工具列表；node_modules 未装会提示先 npm install。**开发阶段写代码/起服务前先调它确认环境**', parameters: { type: 'object', properties: { dir: { type: 'string', description: '项目目录绝对路径' } }, required: ['dir'] } } },
