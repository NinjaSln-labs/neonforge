...
export const TOOL_DEFS = [
  ...
  { type: 'function', function: { name: 'bash', description: '执行命令（需 L3 授权）——启动 dev server（vite / npm run dev / next dev 等）时必须用 --port 指定端口，避开 NeonForge 占用端口 5173 和 5175，用 5176-5199 之间的空闲端口；起服务后把实际端口告诉用户（如「游戏在 http://localhost:5176」）', parameters: { type: 'object', properties: { command: { type: 'string' } }, required: ['command'] } } },
