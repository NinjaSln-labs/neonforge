...
export const TOOL_DEFS = [
  ...
  { type: 'function', function: { name: 'approve-files', description: '批量授权（1-N 文件）：用户**确认执行方案后**，把本次要写/改的文件清单一次性请求批量批准——批准后清单内文件 write/edit 自动放行（不再逐个问）。**不要在执行确认前调用**（批准文件清单 ≠ 确认执行——动手前必须等用户确认执行）；确认执行后调用一次（列出全部文件 + 各自原因）；中途有新文件要改，再调一次补充。', parameters: { type: 'object', properties: { summary: { type: 'string', description: '一句话说明本次要做什么（如「黑屏修复：补 DOM 元素」）' }, files: { type: 'array', items: { type: 'object', properties: { path: { type: 'string', description: '文件路径（绝对路径或项目内相对路径）' }, reason: { type: 'string', description: '为什么新增/修改这个文件（一句话）' } }, required: ['path', 'reason'] } } }, required: ['summary', 'files'] } } },
