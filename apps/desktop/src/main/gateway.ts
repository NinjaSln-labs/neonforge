...
export const TOOL_DEFS = [
  ...
  { type: 'function', function: { name: 'bash', description: '执行命令（需 L3 授权）——启动 dev server（vite / npm run dev / next dev 等）时**用动态端口**：让 vite 自动分配（默认递增）或 --port 0（系统分配）；5173/5175 是 NeonForge（本应用）自己的保留端口（宿主 dev server / 测试 server）——看到它们有服务是宿主本身——你的项目服务用**动态端口**，以起服务实际输出为准。起服务后读实际输出里的地址（如 Local: http://localhost:5174/）或 lsof/curl 确认实际端口，把真实地址告诉用户。', parameters: { type: 'object', properties: { command: { type: 'string' } }, required: ['command'] } } },
  // 2026-08-06 打开网页（用户「帮我打开」催 4 次）：用户说「帮我打开/打开网页」→ 用 open 工具在浏览器打开服务实际地址（无害操作自动放行）
  { type: 'function', function: { name: 'open', description: '打开网页（默认浏览器）——用户说「帮我打开」「打开网页」时调用，传入服务实际地址', parameters: { type: 'object', properties: { url: { type: 'string', description: 'http/https 地址（如 http://localhost:5174/）' } }, required: ['url'] } } },
  ...
  { type: 'function', function: { name: 'approve-files', description: '批量授权（1-N 文件）：用户**确认执行方案后**，把本次要写/改的文件清单一次性请求批量批准——批准后清单内文件 write/edit 自动放行（不再逐个问）。**确认执行后调用**（批准文件清单 ≠ 确认执行——动手前先等用户确认执行）；确认执行后调用一次（列出全部文件 + 各自原因）；中途有新文件要改，再调一次补充。', parameters: { type: 'object', properties: { summary: { type: 'string', description: '一句话说明本次要做什么（如「黑屏修复：补 DOM 元素」）' }, files: { type: 'array', items: { type: 'object', properties: { path: { type: 'string', description: '文件路径（绝对路径或项目内相对路径）' }, reason: { type: 'string', description: '为什么新增/修改这个文件（一句话）' } }, required: ['path', 'reason'] } } }, required: ['summary', 'files'] } } },
  ...
  { type: 'function', function: { name: 'check-capability', description: '检查能力（目标达成前：确认完成目标所需能力就绪——runtime/依赖/工具链）。返回能力清单（平台原生 + 外部扩展 Status）、node 版本、node_modules 是否安装；能力缺失会列出缺什么。**动手产出/起服务前先调它确认能力**；缺失则告知用户并引导安装', parameters: { type: 'object', properties: { dir: { type: 'string', description: '项目目录绝对路径' } }, required: ['dir'] } } },
  // 2026-08-06 设计层升级（服务生命周期独立——用户「白名单匹配不完」）：模型用服务工具管 dev server，不用 bash 起服务/curl 验证
  { type: 'function', function: { name: 'start-server', description: '启动开发服务器（NeonForge 管理进程——自动分配端口并记住地址）。**起服务/打开网页前用它**（用 bash 起服务会端口冲突/进程残留）；参数 dir=项目目录绝对路径，command 可选（npm run dev / npx vite 等，默认 vite）', parameters: { type: 'object', properties: { dir: { type: 'string', description: '项目目录绝对路径' }, command: { type: 'string', description: '启动命令（可选，默认 npx vite）' } }, required: ['dir'] } } },
  { type: 'function', function: { name: 'check-server', description: '检查开发服务器状态——返回 运行中/地址/端口。**验证服务用它**；参数 dir=项目目录绝对路径', parameters: { type: 'object', properties: { dir: { type: 'string', description: '项目目录绝对路径' } }, required: ['dir'] } } },
