...
function waitForUrl(
  ...
  return new Promise((resolve) => {
    ...
    let errBuf = ''
    let done = false
    const finish = (url: string | null, error?: string) => {
      if (!done) {
        done = true
        resolve({ url, error })
      }
    ...
    const timer = setTimeout(() => {
      if (!buf && !errBuf)
        finish(
          null,
          '命令启动超时（15s 无输出）——检查命令是否存在或环境未就绪（可用 check-capability 检测）',
        )
      else finish(null)
    }, timeoutMs)
    ...
    child.on('close', () => {
      if (!buf) {
        const err =
          errBuf.trim() ||
          '命令执行失败（无输出）——检查命令是否存在（如 vite 需先 npm install）或环境是否就绪'
        clearTimeout(timer)
        finish(null, err)
      } else {
        clearTimeout(timer)
...
const SERVER_COMMAND_WHITELIST: Array<{ test: (c: string) => boolean; label: string }> = [
  ...
  {
    test: (c) => /^npm run (dev|start|serve|preview)/.test(c),
    label: 'npm run dev/start/serve/preview',
  },
...
const INSTALL_CMD_RE =
  /(npm|pnpm|yarn|bun) (i|install|add)( |$)|pip install|go mod download|brew install/
...
export async function startServer(
  rootPath: string,
  command?: string,
): Promise<{ ok: true; data: { url: string; port: number } } | { ok: false; error: string }> {
  ...
  try {
    ...
    if (error) {
      ...
      return {
        ok: false,
        error: `start-server: ${error}（已分配端口 ${port}）——可用 check-capability 检测环境后重试`,
      }
    }
    if (!url) {
      // 有输出但没解析到地址（输出慢/格式不同）→ 不杀进程（服务可能正在起），返回启动中——模型用 check-server 确认
      services.set(rootPath, {
        rootPath,
        port,
        pid: child.pid ?? 0,
        url: '',
    ...
    if (HOST_RESERVED_PORTS.has(actualPort)) {
      ...
      return {
        ok: false,
        error: `start-server: 服务落到了宿主保留端口 ${actualPort}（NeonForge 自己用）——已停止；已换显式端口请用 vite 类命令`,
      }
    }
    services.set(rootPath, {
      rootPath,
...
export async function checkServer(
  ...
  | { ok: true; data: { running: boolean; url?: string; port?: number; note?: string } }
...
> {
  ...
  if (!s) {
    return {
      ...
      data: {
        running: false,
        note: '没有启动过的开发服务器——用 start-server 启动（先 check-capability 确认环境）',
      },
    }
  }
  if (!s.url) {
    return {
      ok: true,
      data: { running: false, url: s.url, note: '服务启动中（未确认地址）——稍后再查' },
    }
  }
  try {
