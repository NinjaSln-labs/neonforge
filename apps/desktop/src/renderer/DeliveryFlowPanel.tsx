...
export const STAGE_HINT: Record<string, string> = {
  ...
  需求: '需求阶段只做一件事：把用户真正想要的问清楚。规则：① 一次只问一个问题（从「做什么」开始），不要一次抛多个问题。② 先复述你的理解，然后对需求里的关键词列出 2-3 个候选理解（必须包含同音/近义/模糊词的猜测——如用户说「设计游戏」，「设计」可能打错/听成「射击」「解谜」「建造」）。**候选必须用下面的标记块包裹，每个选项单独一行，行首用 -**：<candidates>\n- 选项一（一句说明）\n- 选项二（一句说明）\n</candidates>\n（块前后可以写引导语，如「你对『设计』的理解，点选或回复序号」）——用户会看到可点击按钮，点选后发送的就是选项文字本身，你也可以让用户直接回复文字/序号。③ 收到用户选择后，**严格按上面 <candidates> 块内列出的顺序理解对应关系**（第一个 - 就是第一个选项），不要重新排序、不要猜测错位；理解后先复述确认（如「你选的是：射击游戏，对吗？」）再继续。④ 用户选定「做什么」后，再逐个问「给谁玩」「在哪儿玩（网页/电脑/手机）」「做成什么样算完」——每个问题的候选同样用 <candidates> 块分行列出（每行一个选项：- 选项（一句说明）），让用户点选或回复。⑤ 用户确认需求后，输出【需求确认：一句话准确需求】，**并判断任务类型**（能力推导——2026-08-06 能力模型：A/B 依据「完成需求所需能力的类别」，不是措辞/复杂度——真实 API 实测：模型曾把「改内容」误判 A）：先想完成这个需求需要什么能力（对照 check-capability 能力视图：系统原生=text-edit 改内容/text-search 搜索/filesystem 整理；外部扩展=node-runtime/python-runtime/dev-tools 建项目/写代码/起服务）——**只需要系统原生能力**（改现有文件内容/整理/移动/重命名/搜索——不建新软件不装东西）→ 输出【任务类型：B 文件操作】（确认后直接动手 edit/bash——不进设计/开发）；**需要外部扩展能力**（要新建代码/软件/页面、起开发服务器——要 node/python runtime + 装依赖）→ 输出【任务类型：A 开发】——类型标注用户可见可改。**B 类（文件操作）需求确认后直接动手执行**（用 read/bash/edit 操作文件——不需要设计技术方案/写代码/部署）——不要套开发流程，然后才提示「点确认推进」——需求确认前不要提示点推进（用户此时会以为需求已确认但实际没确认）。**【需求确认：】标记必须输出**（没有它 UI 无法识别需求已确认——用户点「确认推进」按钮能兜底确认，但标记能保证需求内容准确回写）。⑥ 除 <candidates> 外，**回复正文禁止出现任何尖括号标签**（如 <one-question>、<question> 等）——那些标签会原样显示给用户，用户看到的是标签不是内容；需要强调内容直接写出来就行，不要用标签包裹。本阶段禁止写代码或给技术方案，也不要说「开始动手」这类话（还没到开发）。（2026-08-06 强化·用户「需求阶段不是不会实际动手吗」）**需求阶段绝不实际动手**（不调 write/edit/bash 改东西——工具被门控拦截）——无论任务多简单（如「把 X 改成 Y」）**都必须先输出【需求确认：…】【任务类型：A 开发 / B 文件操作】**并提示「点确认推进」——**不要跳过确认直接说「我把它改成」**（用户还没确认需求，你说改只是空话）**也不要「我把方案说一下」**（简单任务不需要讲方案——需求阶段只确认需求，方案是设计阶段的事；用户确认后直接动手）；用户确认后 B 类直接执行 edit/bash（文件操作）、A 类进设计开发。【需求确认】输出后**必须提示「点确认推进」**——**不要说「进入设计阶段/开始设计」这类话**（阶段切换由用户点推进按钮触发，你说的不算——UI 只认按钮；需求确认后直接宣布进设计会让用户看不到推进提示）。',
