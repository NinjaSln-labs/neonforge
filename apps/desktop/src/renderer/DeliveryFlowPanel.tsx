...
export const STAGE_HINT: Record<string, string> = {
  ...
  需求: '需求阶段只做一件事：把用户真正想要的问清楚。规则：① 一次只问一个问题（从「做什么」开始），不要一次抛多个问题。② 先复述你的理解，然后对需求里的关键词列出 2-3 个候选理解（必须包含同音/近义/模糊词的猜测——如用户说「设计游戏」，「设计」可能打错/听成「射击」「解谜」「建造」）——**每个候选单独一行**（行首编号 + 一句说明），让用户回复序号单选或补充。③ 用户选定「做什么」后，再逐个问「给谁玩」「在哪儿玩（网页/电脑/手机）」「做成什么样算完」——**每个问题的候选选项都分行列出（每行一个选项：编号 + 一句说明），让用户回复序号单选**；绝对不要把选项揉进段落里、也不要用长句把选项连成一段（用户会找不到选项）。④ 用户确认需求后，输出【需求确认：一句话准确需求】，然后才提示「点确认推进」——需求确认前不要提示点推进（此时按钮是禁用的，提示了也点不动）。本阶段禁止写代码或给技术方案，也不要说「开始动手」这类话（还没到开发）。',
  设计: '确认方案、技术选型、页面/结构设计——先定方案，不要急着写代码。2026-08-04 体验修复（用户「设计阶段没确认就进开发」）：设计必须完整输出——整体方案一句话 + 技术选型（用什么做）+ 页面/结构（有哪些界面、每块干什么）+ 模块划分（代码怎么组织）——写完整再让用户点「确认推进」；设计只写了一两句就不要提示确认推进（用户看不到完整设计没法确认，推进了也像没确认）。',
  开发: '直接动手产出真实可运行的文件（用 write/edit 工具；先写出第一版能跑的东西，再问需要用户决策的问题，一次只问一个；不要只提问不产出）。铁律：你的每条回复必须以实际行动收尾——说「开始」后就立刻调用工具干活（read/ls 看目录或 write 写文件），不要把「开始动手了」当结尾停下来等用户，也不要只说话不调工具。2026-08-04 授权架构 v4：动手写文件前，先用一句话列出要修改/新建的文件清单和原因（如「黑屏根因是 DOM 缺失，我要修改 index.html / main.js / style.css」），然后调用 plan_approval 工具把这批文件一次性请求用户批准（用户批准后这些文件的 write/edit 自动放行，不需要逐个确认）——不要直接逐个 write 触发授权；如果中途发现还要改新文件，再调一次 plan_approval 补充。2026-08-04 体验修复：禁止预告轮——说「现在写 X」的同一条回复必须同时调用写 X 的工具（write X），说了就同轮做掉；工具结果显示「已写入：路径」= 该文件已经写好了，不要重复写同一个文件（重复写会被检测为死循环而暂停）；写文件用绝对路径或项目内相对路径。',
  测试: '验证能跑、按验收标准逐项核对',
  部署: '发布/上线（超出数字能力→给指导）',
  交付: '交付包 + 验收对照，确认后关闭'
}
