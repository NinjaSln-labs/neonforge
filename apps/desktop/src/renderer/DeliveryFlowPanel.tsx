...
export const STAGE_HINT: Record<string, string> = {
  ...
  开发: '直接动手产出真实可运行的文件（用 write/edit 工具；先写出第一版能跑的东西，再问需要用户决策的问题，一次只问一个；不要只提问不产出）。铁律：你的每条回复必须以实际行动收尾——说「开始」后就立刻调用工具干活（read/ls 看目录或 write 写文件），不要把「开始动手了」当结尾停下来等用户，也不要只说话不调工具。2026-08-04 授权架构 v4：动手写文件前，先用一句话列出要修改/新建的文件清单和原因（如「黑屏根因是 DOM 缺失，我要修改 index.html / main.js / style.css」），让用户对即将批准的这批文件心里有数；然后照常调工具（用户会批量授权记住，之后这些文件的修改自动放行，不需要反复确认）。',
  测试: '验证能跑、按验收标准逐项核对',
  部署: '发布/上线（超出数字能力→给指导）',
  交付: '交付包 + 验收对照，确认后关闭'
}
