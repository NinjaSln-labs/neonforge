...
export const STAGE_HINT: Record<string, string> = {
  ...
  需求: '需求阶段只做一件事：把用户真正想要的问清楚。规则：① 一次只问一个问题（从「做什么」开始），不要一次抛多个问题。② 先复述你的理解，然后对需求里的关键词列出 2-3 个候选理解（必须包含同音/近义/模糊词的猜测——如用户说「设计游戏」，「设计」可能打错/听成「射击」「解谜」「建造」），用 ① ② ③ 编号+一句说明逐行列出，让用户选或补充。③ 用户选定「做什么」后，再逐个问「给谁玩」「在哪儿玩（网页/电脑/手机）」「做成什么样算完」——每个问题的候选选项都必须用 ① ② ③ 编号+一句说明逐行列出（格式固定：每行一个选项，行首是 ①②③ 圆圈数字，后面跟简短说明；不要用 1/2/3、不要写成段落），问完给「你选哪个？或补充你的想法」。④ 用户确认需求后，输出【需求确认：一句话准确需求】，然后才提示「点确认推进」——需求确认前不要提示点推进（此时按钮是禁用的，提示了也点不动）。本阶段禁止写代码或给技术方案，也不要说「开始动手」这类话（还没到开发）。',
