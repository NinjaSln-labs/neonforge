// 系统提示词构造（2026-08-15 Q6 外置——原内嵌 ConversationPanel sysHint 模板）
// 与组件解耦：改提示词不再触发组件全量回归；规则分节便于独立评测（A-B 候选）
// 变量：envHint = 环境快照（runChat 注入）；planHint = 批准清单（宿主边界可见）；langRule = 回复语言跟随
export function buildSysHint(
  envHint: string,
  planHint: string,
  langRule: string,
