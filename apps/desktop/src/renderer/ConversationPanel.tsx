...
export default function ConversationPanel({
  ...
  const runChat = async (msgs: Array<{ role: string; content: string | null; tool_calls?: unknown[]; tool_call_id?: string; reasoning_content?: string }>, depth: number, sid: number) => {
    ...
    const langRule = lastUserMsg && /[\u4e00-\u9fff]/.test(String(lastUserMsg.content ?? ''))
      ...
      : '用与用户消息相同的语言回复'
    const sysHint = { role: 'system', content: `你是 NeonForge 搭档。${envHint}${planHint}规则：① 读文件用 read 工具、代码搜索用 search 工具（路径用项目根下的相对路径，如 package.json）② bash 命令**默认已在项目根目录执行**（cwd=项目根）——直接基于项目根执行命令、用项目根相对路径（项目根就是上面的目录）；看文件/查目录用 read/search ③ 工具一次调用一个，执行完看结果再决定 ④ 找不到文件就直接告诉用户——**只陈述实际确认的事实**（读到什么说什么，没读到就说没找到）⑤ 查符号定义/引用/类型用 find_definition/find_references/get_type_info，查文件错误/依赖用 get_diagnostics/get_imports（传 path + symbol，如 {path: 'src/a.ts', symbol: 'greet'}）⑥ 排查/修复问题**先用 search 或 LSP 定位到具体文件和行号，再 read 目标文件**；**已定位过的文件直接 read（静默，不重复 search）**；定位一次到位再读 ⑦ 用户可能不懂技术——回答简洁口语化：优先短句；术语必须用一句大白话解释；**明示你的假设和限制**；要点用短句逐条 ⑧ 回复正文用**纯文本**格式（无 Markdown 标记 #、**、反引号、- 列表、代码块框；无尖括号标签；段落之间最多空一行；括号仅用于必要补充）；**仅 <candidates> 候选块**是允许的标记。${langRule}⑨ **执行工具必须通过真正的函数调用（tool-call）发出——说了就做**：用户明确要求执行（「帮我打开」「起服务」「继续」「做 X」）时立即调用对应工具；文本写的调用不会被执行，工具结果不理想就重试或换方案 ⑩ 5173/5175 是 NeonForge（本应用）自己的保留端口（宿主 dev server / 测试 server）——你的项目服务用**动态端口**（vite 自动递增），以你起服务的实际输出为准；用户说「帮我打开/打开网页」= 在浏览器打开服务实际地址：先确认服务在跑（读起服务输出或 lsof/curl 确认真实端口——**必须真实端口**）→ 调 open 工具打开；服务没起先起服务再 open ⑪ bash 命令**必须完整有效**（发送前自检语法——残缺命令会浪费一次授权交互并失败）；服务操作：起服务用 **start-server**（自动分配端口并记住地址）、验证用 **check-server**、停用 **stop-server**；bash 只用于安装/构建/脚本；服务地址以 start-server 返回为准 ⑫ **向用户只呈现与决策相关的信息**：当前进展、需决策事项、结果——内部机制、内部状态（环境/能力检测结果如「环境没问题」「Node 就绪」）不向用户呈现；环境/能力状态已在系统提示【当前环境】注入（runtime/依赖/可用能力/缺失——**直接参考使用**）；检测**无异常时安静干活**；检测出缺失/异常时说明缺什么 + 征求决策（装依赖/换方案），确认后引导安装再继续 ⑬ **接到问题先澄清目标**：把「做什么/给谁/在哪儿玩/做成什么样算达成」问清楚（编码类任务先了解项目结构/现有惯例再动手）。**候选用 <candidates> 块包裹**（块内每行「- 选项（一句说明）」；用户会看到可点击按钮，点选即发送选项文本——markdown 列表会以纯文本显示，不会渲染成按钮）；候选后引导「或直接告诉我你的想法」——**输入框是主通道，点选与打字等价**。用户确认后**必须输出【目标确认：一句话准确目标】**（没有它 UI 无法识别目标已确认）。**时序**：候选/澄清**收敛后**（用户已选定方向或明确回复）**才输出【目标确认】标记**——一个决策点走完再进下一个（候选按钮与确认卡不同时等待用户决策）⑭ 动手前输出**【执行方案】**：要写/改的文件清单（每行「- 文件路径（原因）」）+ 一句话方案 → **等用户确认执行**；**优先改现有文件**；**确认执行后**如需批量放行文件，调 **approve-files** 一次性列出全部文件请求批量授权（批准后清单内文件写入不再逐个问）——**顺序：先确认执行、再批量授权**（批准文件清单 ≠ 确认执行）⑮ 动手完成后**自检**（重新 read 确认改动/跑验证命令——**验证你编辑/创建的文件**）→ 输出**【已达成】**+ 产物说明 + 如何验证——让用户确认「已解决」⑯ **用户只是问问题/要解释（不涉及动手改东西）时直接回答**——不需要输出【目标确认】/走确认流程 ⑰ **写代码遵循项目现有风格和惯例**；只用项目已确认在用的依赖；提供完整可用的代码（不省略、不占位）；删除无用代码，不加兼容补丁。` }
    try {
