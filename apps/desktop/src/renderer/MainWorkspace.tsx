...
function TaskPanel() {
  const [preheatInfo, setPreheatInfo] = useState<{
    cache: { hash: string; history: Array<{ hit: boolean }> } | null
  } | null>(null)
  useEffect(() => {
  ...
  return (
    <div className="nf-task">
      ...
      <details>
        ...
        <p className="nf-meta">
          {preheatInfo?.cache ? `前缀缓存 ${hitRate}% 命中（hash ${preheatInfo.cache.hash}）` : '—'}
        </p>
      </details>
    </div>
  )
...
export default function MainWorkspace({
  ...
  const refreshProjectFiles = () => {
    if (!rootPath) return
    void window.neonforge.workspace
      .listDir(rootPath)
      .then((entries) =>
  ...
  const handleToolResult = (r: { name: string; file?: string; ok: boolean }) => {
    ...
    setRealChanges((prev) =>
      prev.some((c) => c.file === r.file)
        ? prev
        : [...prev, { file: r.file as string, op: r.name }],
    )
    ...
    if (activeProblem) {
      setProblems((prev) =>
        prev.map((p) => {
          ...
          const rawAuth = (p.snapshot?.authorized ?? []) as Array<
            { tool: string; file: string } | string
          >
          ...
          if (!alreadyFile)
            tlog('problem.snapshot_updated', { problemId: activeProblem, field: 'authorized' })
  ...
  const tlog = (type: string, detail: Record<string, unknown>) => {
    try {
      void window.neonforge.timeline?.log?.({
        session: sessionIdRef.current || (rootPath ?? undefined),
        type,
        role: 'system',
        detail,
      })
    ...
    if (activeProblem) {
      setProblems((prev) =>
        prev.map((p) =>
          p.id === activeProblem
            ? {
                ...updateProblemSnapshot(p, { goal: title }),
  ...
  const handleUserMessage = (text: string) => {
    ...
    tlog(dup ? 'problem.rerun' : 'problem.created', {
      problemId: dup ? dup.id : inst.id,
      title: inst.title,
    })
    ...
    setProblems((prev) => {
      if (dup) {
        ...
        return [
          { ...snap, updatedAt: inst.updatedAt, status: 'executing' as const },
          ...prev.filter((x) => x.id !== dup.id),
        ]
      }
      return [inst, ...prev]
    })
  ...
  const handleConfirmClosed = () => {
    ...
    setProblems((prev) =>
      prev.map((p) => (p.id === activeProblem ? { ...p, status: 'closed' as const } : p)),
    )
  ...
  return (
    <div className="nf-app">
      ...
      <aside className="nf-panel nf-panel--center">
        <header className="nf-panel__header">
          ...
          <div className="nf-chat__actions">
            <button
              type="button"
              className="nf-session__new"
              onClick={() => setShowSettings((v) => !v)}
            >
              <IconSettings size={12} /> 设置
            </button>
            <button type="button" className="nf-session__new" onClick={onBackStart}>
              启动页
            </button>
          </div>
        </header>
        ...
        <div className="nf-panel__body">
          {chatTab === 'chat' ? (
            <ConversationPanel
              ...
              onApprovalChange={setPendingApproval}
              onActionPromiseHint={setActionHint}
              externalRequest={rerunRequest}
              onExternalConsumed={() => setRerunRequest(null)}
              onToolResult={handleToolResult}
              onUserMessage={handleUserMessage}
              onGoalConfirmed={handleGoalConfirmed}
              onPlanConfirmed={handlePlanConfirmed}
              onGoalRejected={handleGoalRejected}
              onPlanRejected={handleExecutionRejected}
              goalConfirmed={goalConfirmed}
              planConfirmed={planConfirmed}
              goalSeq={goalSeq}
              recentFilesExternal={projectFiles}
              initialPrompt={initialPrompt}
              onSessionStart={handleSessionStart}
              activeAuthorizedLogs={
                problems.find((p) => p.id === activeProblem)?.snapshot?.authorized
              }
            />
          ) : (
            <TaskPanel />
      ...
      <OutputPanel
        ...
        onAdjustProblem={() =>
          setChatTab('chat')
        } /* 2026-08-03 A2 审计修复：继续调整 → 切回对话 Tab（原空函数无反应） */
