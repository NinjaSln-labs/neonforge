...
export function isConfirmIntent(t: string): boolean {
  ...
  // #6 真机 2026-08-30（P2-4）：「已解决」入表——sysPrompt ⑮ 引导用户回「已解决」，
  // 原词表缺位 → resolution pending 期落入 reject 分支（隐式拒绝——真机实测无效果）
  if (
    /^(行|好|可以|确认|同意|对|没错|没问题|就这么办|按这个方案|继续|动手吧|开始吧|写吧|就按你说的|已解决|解决了)[。！!~～]?$/.test(
      s,
    )
  )
    return true
