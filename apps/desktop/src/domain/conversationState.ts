...
/** 动作属性判定依据（审计） */
export type ActionBasis = 'tool-type' | 'command-head' | 'command-chain' | 'git-subcommand' | 'plan-list'
...
export function approvalDecided(
  ...
  if (!decision.confirm) {
    ...
    next.deniedApprovals = [...s.deniedApprovals, { toolName: request.toolName, subject: request.subject }]
  }
  return next
}

// 兼容壳（S3 由 userDecided 直连取代——renderer 现状消费；缺省拒绝原因仅兼容旧调用，新代码一律显式带原因）
export function userConfirmed(s: ConversationState, point: 'goal' | 'plan' | 'resolution'): ConversationState {
  return userDecided(s, point, { confirm: true })
}

export function userRejected(s: ConversationState, point: 'goal' | 'plan' | 'resolution', reason?: RejectReason): ConversationState {
  return userDecided(s, point, { confirm: false, reason: reason ?? { kind: 'other', text: '用户拒绝' } })
}
...
const BASH_CHAIN_DANGEROUS = /\b(rm|mv|cp|mkdir|touch|npm|pnpm|yarn|git|curl|wget|python|python3|node|install|unlink|ln|chmod|chown)\b/

// git 只读子命令（Codex is_safe_git_command 方向）
const GIT_READONLY_SUBCOMMANDS: ReadonlySet<string> = new Set(['status', 'log', 'diff', 'show', 'branch', 'ls-files', 'remote'])
...
export function classifyReadonly(name: string, command?: string): ActionKind {
  ...
  if (netMatch) {
    ...
    const hasBodyFlag = /-d\b|--data\b|--data-raw\b|-F\b|--form\b|-X\s+(POST|PUT|PATCH|DELETE)\b/.test(c)
    if (!hasBodyFlag && (!method || method[1] === 'GET' || method[1] === 'HEAD')) return 'network-read'
    return 'hazardous'
  }
  // 重定向到文件 → 写副作用
  ...
  if (segments.length > 1 && segments.some((seg) => BASH_CHAIN_DANGEROUS.test(seg))) return 'hazardous'
...
export function sessionGate(s: ConversationState, action: ExecAction): { ok: boolean; reason: string } {
  if (s.pending !== 'none') {
  ...
  if ((action.name === 'write' || action.name === 'edit') && s.plannedFiles.size > 0 && !inPlannedFiles(s, action)) {
...
export function actionGate(action: ExecAction, inPlanned: boolean, policy?: ActionGatePolicy): ActionGateResult {
  const kind = classifyReadonly(action.name, action.command)
  if (kind === 'readonly') {
  ...
  if (kind === 'network-read') {
    ...
    if (allowLocal && localhost) {
      return { verdict: 'allow', attribute: { kind: 'network-read', basis: 'command-head' }, risk: 'low' }
    }
    return { verdict: 'ask', attribute: { kind: 'network-read', basis: 'command-head' }, risk: 'medium' }
  }
  if (action.name === 'write' || action.name === 'edit') {
    if (inPlanned) {
      return { verdict: 'allow', attribute: { kind: 'in-plan', basis: 'plan-list' }, risk: 'low' }
    }
    return { verdict: 'ask', attribute: { kind: 'out-of-plan', basis: 'plan-list' }, risk: 'medium' }
  }
  // hazardous（bash 写/未知）
  const hazardous = policy?.hazardous ?? 'ask'
  return { verdict: hazardous, attribute: { kind: 'hazardous', basis: 'command-chain' }, risk: 'high' }
}
...
export function canExecute(s: ConversationState, action: ExecAction, inPlanned: boolean, policy?: ActionGatePolicy): { ok: boolean; reason: string } {
  const gate = sessionGate(s, action)
  if (!gate.ok) return gate
  // 清单为空（无计划）→ 无文件边界（写放行由确认点把关——继承现状语义）
  const effectiveInPlanned = (action.name === 'write' || action.name === 'edit') && s.plannedFiles.size === 0 ? true : inPlanned
  const verdict = actionGate(action, effectiveInPlanned, policy)
  if (verdict.verdict === 'deny') return { ok: false, reason: `动作被策略拦截（${verdict.attribute.kind}）` }
  return { ok: true, reason: '' }
}
...
export function deriveDecisionPoint(
  ...
  if (!state.goalConfirmed && (proposals.goal !== undefined || userRequested === 'goal')) return 'goal'
  // 2. 目标已确认 && 方案提议存在（或用户主动请求方案确认）&& 方案未确认 → plan
  if (state.goalConfirmed && !state.planConfirmed && (proposals.plan !== undefined || userRequested === 'plan')) return 'plan'
  // 3. 目标+方案已确认 && 存在需授权动作 → approval（不变量 3 同源：动作属性判定）
  if (state.goalConfirmed && state.planConfirmed && pendingActions.some(actionNeedsApproval)) return 'approval'
 ...
 */
export function verifyCompletion(claim: CompletionClaim): { ok: boolean; missing: string[]; unverifiable: string[] } {
...
export function decideProgressGuarantee(s: ConversationState, turn: TurnProgress): ProgressGuaranteeDecision {
  ...
  // 已有推进（产出/提议/证据）→ 不强制
  if (turn.produced || turn.proposed || turn.providedEvidence) return { mode: 'auto', reason: 'has-progress' }
  // 确认后无任何推进：工具可用 → 逼工具产出（原 required）；工具不可用 → 逼「推进」（允许提议/证据/提问——不逼调工具）
  if (turn.toolsAvailable) return { mode: 'require-action', reason: 'confirmed-no-progress-tools-available' }
...
export function pendingCardToShow(
  ...
  if (goalConfirmed && !planConfirmed && (
