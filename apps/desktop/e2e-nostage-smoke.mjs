  ...
  async waitSettled(timeoutMs = 120000) {
    ...
    while (Date.now() < hardDeadline) {
      ...
      if (last) {
        log(
          '⚠️',
          `waitSettled 窗口耗尽（${timeoutMs / 1000}s）——返回最后一条已知回复（${last.content.slice(0, 30)}…）`,
        )
        return last
      }
      windowEnd = Date.now() + 30000
    ...
    throw new Error(
      `waitSettled 硬超时 ${(timeoutMs * 4) / 1000}s（状态栏="${sb}"，最后消息="${last?.content.slice(0, 40) ?? ''}"）`,
    )
  }
...
function checkObservations(content, toolsText, obs) {
  ...
  if (!obs.O1_goalConfirm.hit && /【目标确认|目标确认：/.test(content)) {
    obs.O1_goalConfirm.hit = true
    obs.O1_goalConfirm.at = content.slice(0, 60)
  }
  if (!obs.O3_execPlan.hit && /【执行方案/.test(content)) {
    obs.O3_execPlan.hit = true
    obs.O3_execPlan.at = content.slice(0, 80)
  }
  if (!obs.O5_doneReport.hit && /【已达成/.test(content)) {
    obs.O5_doneReport.hit = true
    obs.O5_doneReport.at = content.slice(0, 60)
  }
  if (
    !obs.O4_produced.hit &&
    /已写入|已修改|写入|修改.*成功/.test(toolsText) &&
    /write|edit/.test(toolsText)
  ) {
    obs.O4_produced.hit = true
    obs.O4_produced.at = toolsText.slice(0, 80)
  }
}
...
async function main() {
  ...
  try {
    ...
    page.on('console', (m) => {
      const t = m.text()
      if (t.includes('[debug]') || t.includes('[conversation]'))
    ...
    // 从零开始 + 输入需求（场景 A：起始页填）
    await page
      .locator('.nf-start__input')
      .fill('我想做一个3D设计游戏，网页打开就能玩，发给朋友玩，先做个能玩的版本')
    ...
    while (Date.now() < deadline && rounds < MAX_ROUNDS) {
      ...
      if (msg.content === lastProcessed) {
        ...
        const aprox = await page
          .locator(
            '.nf-toolcall__approve, .nf-toolcall__batch-approve, .nf-toolcall__remember, .nf-toolcall__approveall',
          )
          .count()
          .catch(() => 0)
        const busyDots = await page
          .locator('.nf-statusbar__dot--working')
          .count()
          .catch(() => 0)
        const runCards = await page
          .locator('.nf-toolcall--running, .nf-toolcall--pending')
          .count()
          .catch(() => 0)
      ...
      console.log(`\n── 轮 ${rounds} ──`)
      console.log(
        `  🤖 模型：${msg.content.slice(0, 160).replace(/\n/g, ' ')}${msg.content.length > 160 ? '…' : ''}`,
      )
      if (msg.candidates.length > 0)
        console.log(`  🔘 候选：${msg.candidates.slice(0, 4).join(' | ')}`)
      ...
      if (
        /(确认一下|你的想法|再动手|定下来|先确认|想确认|了解一下|范围定)/.test(msg.content) &&
        !/(【目标确认|【执行方案|【已达成)/.test(msg.content)
      ) {
      ...
      if (
        /[?？]/.test(msg.content) &&
        /(做什么|哪种|给谁|在哪儿|算完|什么样|理解|确认|对吗|好吗|行吗|要不要|还是|具体|玩法)/.test(
          msg.content,
        )
      ) {
        const c = msg.content
        const ans = /(玩法|做什么|哪种|什么样|具体|怎么玩)/.test(c)
          ? '搭积木玩法，能放方块、拖拽、上色'
          : /(给谁|谁玩|朋友|自己)/.test(c)
            ? '发给朋友玩'
            : /(在哪儿|哪里|电脑|手机|网页|平台)/.test(c)
              ? '网页打开就能玩'
              : /(可以吗|确认|对吗|好吗|行吗|点头|同意|方案)/.test(c)
                ? '可以'
                : execConfirmed
                  ? '可以，继续'
                  : '你按合适的来'
      ...
      // 4. 模型陈述/汇报——
      if (
        /【已达成|已达成|全部完成|都完成了|已经完成|做完了|写好了|写完了|部署好了|修好了/.test(
          msg.content,
        )
      ) {
        log('✅', '模型完成汇报——尝试收尾（O5）')
      ...
      if (
        /说要做但还没动手|回复.*继续|现在写入|马上|这就|直接开写|开始写|准备写/.test(msg.content)
      ) {
