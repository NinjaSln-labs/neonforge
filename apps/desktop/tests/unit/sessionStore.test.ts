...
import { serializeMessages, saveSession, loadSession, clearSession, SESSION_KEY, SESSION_MAX } from '../../src/renderer/sessionStore'
...
const DONE_MSG = { role: 'assistant' as const, content: '已处理', reasoning: '思考', status: 'done' as const }
...
describe('sessionStore（断点续做——会话持久化）', () => {
  ...
  it('serializeMessages：toolCalls 仅完整状态', () => {
    const msgs = [
      DONE_MSG,
      { role: 'assistant' as const, content: '', status: 'done' as const, toolCalls: [
  ...
  it('saveSession → loadSession 往返', () => {
    saveSession([{ role: 'user', content: '你好' }, { role: 'assistant', content: '收到' }])
  ...
  it('serializeMessages：超过上限 → 截取末尾', () => {
    const msgs = Array.from({ length: SESSION_MAX + 10 }, (_, i) => ({ role: 'user' as const, content: `msg ${i}` }))
    const out = serializeMessages(msgs)
