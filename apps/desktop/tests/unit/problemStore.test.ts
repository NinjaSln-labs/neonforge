...
import {
  ...
  PROBLEMS_KEY,
  PROBLEMS_MAX,
} from '../../src/renderer/problemStore'
...
describe('problemStore（问题台账——创建/持久化）', () => {
  ...
  it('loadProblems：空存储 → fallback', () => {
    ...
    expect(
      loadProblems([{ id: 'x', title: 't', status: 'closed', updatedAt: '10:00' }]),
    ).toHaveLength(1)
  })
...
describe('问题会话快照（基线 §21 断点续做深度——2026-08-02 增强）', () => {
  ...
  it('updateProblemSnapshot：替换语义（patch 整字段覆盖；调用方读取+追加）', () => {
    ...
    p = updateProblemSnapshot(p, {
      authorized: [...(p.snapshot?.authorized ?? []), '[write] /tmp/b.txt'],
      pending: ['确认分类'],
    })
