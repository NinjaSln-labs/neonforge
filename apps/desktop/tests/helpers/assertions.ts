...
export async function expectAtLeast(
  ...
  await expect
    .poll(async () => (await locator.count()) >= count, {
      timeout,
      message: `期望至少 ${count} 个匹配元素`,
    })
    .toBe(true)
}
...
/** 最近一条用户消息包含文本 */
export async function expectLastUserMsg(
  page: Page,
  text: string | RegExp,
  timeout = 8000,
): Promise<void> {
...
/** 存在用户消息包含文本 */
export async function expectUserMsg(
  page: Page,
  text: string | RegExp,
  timeout = 8000,
): Promise<void> {
...
export async function expectAssistantMsg(
  page: Page,
  text: string | RegExp,
  timeout = 8000,
): Promise<void> {
  await expectText(
    page.locator('.nf-msg--assistant .nf-msg__body').filter({ hasText: text }),
    text,
    timeout,
  )
}
...
export async function expectToolCallState(
  page: Page,
  state: 'done' | 'need-approval' | 'plan-approval',
  count: number,
  timeout = 15000,
): Promise<void> {
