...
### 3.4 执行确认的活动边界（工具级放行清单——审计 Major 1 闭合）

**未确认执行时**（模型只给方案——信息类放行、执行类挂起）：

| 工具类 | 动作 | 理由 |
|--------|------|------|
| read / search / LSP 工具 | **放行** | 信息类——模型读文件/查能力/给方案的前提 |
| check-capability | **放行** | 能力检查——确认目标后给方案的必要步骤 |
| plan_approval | **放行** | 文件清单提议——不执行、只声明边界 |
| write / edit / bash | **挂起**（pending——不执行——工具卡保持等待）| 执行类——模型停住等用户确认执行（像授权卡：不批准不执行）|

**已确认执行后**：全部放行（再按计划清单判定——文件级边界）。

### 3.5 门控优先级（执行确认 vs 计划清单——审计 Major 2 闭合）

```
执行未确认 ─→ 全部执行动作（write/edit/bash）挂起——不管文件在不在计划清单
    │（用户点「确认执行」）
    ▼
执行已确认 ─→ 按计划清单判定：
    ├─ file ∈ 清单 → 放行执行
...
**优先级规则**：执行确认是**执行级**边界（先判定——未确认则全部挂起）；计划清单是**文件级**边界（后判定——已确认执行后逐文件核对）。两个边界不互斥——执行未确认时不进入清单判定。
