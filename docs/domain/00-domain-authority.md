# 00 — 领域权威总纲（无阶段·目标驱动版 v4.0）

> 版本：v4.0 · 2026-08-16（v3.0 · 2026-08-07）
...
**tool_choice 传递**：`required`（确认后无推进时强制模型推进——产出/提议/证据）/ `auto`（澄清/等用户决策/失败诊断）——推进保障策略（§4）的输出经网关传递。
...
| 目标+方案确认、无产出且工具可用 | **require-action**（要求行动——调工具产出；与 require-advance 同映射 tool_choice='required'——区别在系统提示措辞：行动 vs 推进；2026-08-16 第 16 轮审计 #2 补——设计 §3.3/04 §3.1/07 §1.1 两档对齐）|
