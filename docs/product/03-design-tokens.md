...
| 令牌 | 深色值 | 用途 |
|------|--------|------|
| `--nf-bg` | `#120F1E` | 应用主背景（中性深紫，AI Agent IDE 深空底） |
| `--nf-bg-panel` | `#1A1530` | 面板背景（会话区、对话区、产物区） |
| `--nf-bg-elevated` | `#262044` | 浮面（输入框、消息、悬停态） |
| `--nf-bg-skeleton` | `#3E3E3E` | 骨架屏占位底（2026-08-16 第 17 轮审计 #1 补——02 §11 Skeleton 引用） |
| `--nf-accent-soft` | `#2B2A52` | 深靛（选中态背景、思考中 halo 底） |
| `--nf-border` | `#2E2547` | 默认边框 |

### 气泡背景

| 令牌 | 深色值 | 用途 |
|------|--------|------|
| `--nf-bubble-user` | `rgba(99,102,241,0.18)` | 用户消息气泡（靛蓝淡彩） |
| `--nf-bubble-partner` | `rgba(99,102,241,0.08)` | 搭档消息气泡（靛蓝淡彩底） |
...
### 强调色（赛博朋克三 accent：靛 / 紫 / 橙）

| 令牌 | 值 | 用途 |
|------|-----|------|
| `--nf-accent` | `#6366F1` | 靛蓝：主 accent（按钮、聚焦、思考 halo、链接） |
| `--nf-violet` | `#A855F7` | 紫：渐变端（主渐变 靛→紫） |
| `--nf-amber` | `#FF8A00` | 橙：行动色（agent 行动节点、待批准、高亮） |
| `--nf-cyan` | `#22D3EE` | 青：对撞（行动进度/高亮点缀） |
| `--nf-green` | `#22C55E` | 成功/接受：diff 新增行、任务完成 |
...
| 令牌 | 值 | 用途 |
|------|-----|------|
| `--nf-grad-primary` | `linear-gradient(90deg, #6366F1, #A855F7)` | 主按钮、品牌渐变 |
| `--nf-grad-neon` | `linear-gradient(90deg, #6366F1, #A855F7, #FF8A00)` | 输入框聚焦边框（靛→紫→橙霓虹） |
| `--nf-glow` | `0 0 12px rgba(99,102,241,0.30), 0 0 32px rgba(255,138,0,0.18)` | 霓虹光晕（聚焦态） |

### 文字色

| 令牌 | 深色值 | 用途 |
|------|--------|------|
| `--nf-text-primary` | `#F3F0FF` | 主文字：消息正文、标题 |
| `--nf-text-secondary` | `#A99EC4` | 次要文字：副标题、状态文字、时间戳 |
| `--nf-text-tertiary` | `#6E6590` | 占位文字、禁用态文字 |
